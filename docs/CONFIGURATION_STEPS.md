...
- Check the **Language** column as well as the status. A `_hi` template must say **Hindi**; if it says English the Language dropdown was left on English at creation and the code's send (language code `hi`) will not match. Do not delete it — Meta reserves a deleted template's name for 30 days. Add a Hindi version under the same name instead (open the template → **Add language** / **Add translation** → Hindi → same content). Source: https://respond.io/help/whatsapp/whatsapp-message-templates (30-day rule), https://www.twilio.com/en-us/changelog/whatsapp-template-console-redesign---translation-support (translations under one name).
- All **six** must show **Active** with the right language before Step 4. The code falls back to the `_en` template for any employee whose language has no approved template, so if a `_hi` one is rejected, fix and resubmit it; do not switch the flag on with an `_en` one missing.
