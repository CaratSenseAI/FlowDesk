...
import { ShieldAlert, MessageCircle, CheckCheck, Clock3, Sparkles, X } from 'lucide-react';
    ...
    >
      <div className="flex items-center justify-between px-4 py-3 border-b border-ink-200 dark:border-white/[.06]">
        ...
        <div className="flex items-center gap-1">
          ...
          <button onClick={onClose} className="btn-ghost h-7 w-7 p-0 justify-center"><X className="h-4 w-4" /></button>
        </div>
      </div>
      ...
      <ul className="max-h-[60vh] overflow-y-auto divide-y divide-ink-100 dark:divide-white/[.04]">
        {notifications.map((n) => {
          ...
          return (
            <li key={n.id} className={`flex items-start gap-3 px-4 py-3 ${n.unread ? 'bg-brand-50/30 dark:bg-brand-500/5' : ''}`}>
              ...
              {n.unread && <span className="mt-2 h-2 w-2 rounded-full bg-brand-500" />}
            </li>
          );
        })}
      </ul>
