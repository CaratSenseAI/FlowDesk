...
export default function CreateTaskModal({ open, onClose }) {
  ...
  const [deadline, setDeadline] = useState(() => new Date(Date.now() + 3 * 24 * 60 * 60 * 1000).toISOString().slice(0, 10));
  ...
  const submit = (e) => {
    ...
    const customFields = fields.reduce((acc, f) => (f.key ? { ...acc, [f.key]: f.value } : acc), {});
    addTask({
    ...
    <Modal
      ...
      footer={
        <>
          ...
          <button className="btn-primary" onClick={submit} disabled={!title || !assignedTo}>
    ...
    >
      <form onSubmit={submit} className="space-y-4">
        <div>
          ...
          <input className="input" value={title} onChange={(e) => setTitle(e.target.value)} placeholder="e.g. Reconcile failed refunds" />
        </div>
        <div>
          <label className="label">Description</label>
          <textarea rows={3} className="input" value={description} onChange={(e) => setDescription(e.target.value)} placeholder="Provide context, links, or expectations…" />
        </div>
        ...
        <div>
          <div className="flex items-center justify-between mb-2">
            ...
            <button type="button" onClick={() => setFields((f) => [...f, { key: '', value: '' }])} className="text-xs font-semibold text-brand-600 hover:text-brand-700 inline-flex items-center gap-1">
            ...
            {fields.map((f, i) => (
              <div key={i} className="grid grid-cols-12 gap-2 items-center">
                ...
                <button
                  ...
                  className="btn-ghost h-10 w-10 p-0 col-span-1 justify-center"
                >
