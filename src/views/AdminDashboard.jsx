  ...
  return (
    <div className="space-y-5">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
        ...
        <TrackerDetailCard onSeeAll={() => onNavigate?.('analytics')} />
        <ChatCard onSeeAll={() => onNavigate?.('whatsapp')} />
      </div>

      <div className="grid grid-cols-1 lg:grid-cols-2 gap-5">
        <UpcomingDeadlinesCard tasks={tasks} onOpen={onOpenTask} onSeeAll={() => onNavigate?.('tasks')} />
        <WorkloadCard tasks={tasks} users={users} onSeeAll={() => onNavigate?.('team')} />
      </div>
    </div>
  );
}
